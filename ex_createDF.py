...
#MORE-Q-G1
def get_OM_df(path='./exMORE_Q_G1.hdf5'):
...
def get_REC_df(path='./exMORE_Q_G1.hdf5'):
    with h5py.File(path,'r') as f:
...
#MORE-Q-G2
def get_XTB_df(path='./exMORE_Q_G2.hdf5',sys='REC'): 
    with h5py.File(path,'r') as f:
        data = []
...
def get_XTB_ORCA_df(path='./exMORE_Q_G2.hdf5',sys='REC'): 
    with h5py.File(path,'r') as f:
        data = []
            ...
            for xx_om in xx_oms:
                for xx_dm in list(f['/']['ORCA'][xx_rec][xx_om].keys()):
                    ...
                    for xx_prop in xx_props:
                        ...
                        #print(xx_rec,xx_om,xx_dm,xx_prop)
...
#MORE-Q-G3
def get_VASP_df(path='./exMORE_Q_G3.hdf5',sys='CPLX'):
    with h5py.File(path,'r') as f:
        ...
        #print(props)
        df = pd.DataFrame(columns=list(['REC','OM','DM'] + props)) #total range should be user defined
        data = []
        for xx_rec in tqdm(xx_recs):
            for xx_om in xx_oms:
                for xx_dm in list(f['/'][f'{sys}'][xx_rec][xx_om].keys()):
                    _data = []
                    for xx_prop in props:
                        _data.append(f['/'][sys][xx_rec][xx_om][xx_dm][xx_prop][...])
                    data.append([int(xx_rec.split('_')[0]),int(xx_om.split('_')[0]),int(xx_dm.split('_')[0])] + _data)
        df = pd.DataFrame(data,columns=['REC','OM','DM'] + props)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-ds',type=str,help="Type anyone of the following: 'G1_OM', 'G2_REC', 'G2_XTB_OM', 'G2_XTB_REC', 'G2_XTB_DM', 'G2_XTB_ORCA_OM', 'G2_XTB_ORCA_REC', 'G2_XTB_ORCA_DM', 'G3_VASP_CPLX', 'G3_VASP_OM', 'G3_VASP_SUB'")
